...
//Milan
  ...
  void buildDBranch(TTree* dnt, bool D0kpimode=true, bool detailMode=false)
  {
    ...
    std::cout<<"D0kpimode = "<<D0kpimode<<" detailMode = "<<detailMode<<std::endl;
    ...
    dnt->Branch("candSize",&Dsize);
    dnt->Branch("centrality",&centrality);
    dnt->Branch("HFsumET",&HFsumET);
    ...
    dnt->Branch("phi",Dphi,"Dphi[Dsize]/F");
    //dnt->Branch("Dy",Dy,"Dy[Dsize]/F");
    dnt->Branch("y",Dy,"Dy[Dsize]/F");
    ...
    dnt->Branch("2DPointingAngle",Dalpha2D,"Dalpha2D[Dsize]/F");
    dnt->Branch("mva",Dmva,"Dmva[Dsize]/F");
    ...
    dnt->Branch("dca",Ddca,"Ddca[Dsize]/F");
    ...
    dnt->Branch("Dtrk1Chi2n",Dtrk1Chi2n,"Dtrk1Chi2n[Dsize]/F");
    dnt->Branch("Dtrk2Chi2n",Dtrk2Chi2n,"Dtrk2Chi2n[Dsize]/F");
        ...
        if(isDchannel[t]==1)
          {
            for(int j=0;j<DInfo->size;j++)
              ...
              //float lvtxMag = TMath::Sqrt((DInfo->vtxX[j]-EvtInfo->PVx)*(DInfo->vtxX[j]-EvtInfo->PVx)+(DInfo->vtxY[j]-EvtInfo->PVy)*(DInfo->vtxY[j]-EvtInfo->PVy)+(DInfo->vtxZ[j]-EvtInfo->PVz)*(DInfo->vtxZ[j]-EvtInfo->PVz));  //DInfo->lvtxMag[j];
                //float rvtxMag = TMath::Sqrt((DInfo->vtxX[j]-EvtInfo->PVx)*(DInfo->vtxX[j]-EvtInfo->PVx)+(DInfo->vtxY[j]-EvtInfo->PVy)*(DInfo->vtxY[j]-EvtInfo->PVy)); //DInfo->rvtxMag[j];
                //float sigmaLvtxMag = TMath::Sqrt(DInfo->vtxXErr[j]*DInfo->vtxXErr[j]+DInfo->vtxYErr[j]*DInfo->vtxYErr[j]+DInfo->vtxZErr[j]*DInfo->vtxZErr[j]);
                //float sigmaRvtxMag = TMath::Sqrt(DInfo->vtxXErr[j]*DInfo->vtxXErr[j]+DInfo->vtxYErr[j]*DInfo->vtxYErr[j]);
                ...
                //if( (fabs(TrackInfo->eta[DInfo->rftk1_index[j]] - TrackInfo->eta[DInfo->rftk2_index[j]])) > 1.0  ) continue;
                if (VtxProb < 0.02) continue;
               //std::cout<<DInfo->pt[j]<<" "<<lvtxMag<<" "<<DInfo->lvtxMag[j]<<" "<< " "<<rvtxMag<<" "<<DInfo->rvtxMag[j]<<std::endl;
                if (DInfo->lvtxMag[j]*DInfo->lvtxMag[j]/DInfo->sigmaLvtxMag[j]<3.0) continue;
                if (DInfo->rvtxMag[j]*DInfo->rvtxMag[j]/DInfo->sigmaRvtxMag[j]<2.0) continue;
                //if (TMath::Sqrt((DInfo->vtxX[j]-EvtInfo->PVx)*(DInfo->vtxX[j]-EvtInfo->PVx)+(DInfo->vtxY[j]-EvtInfo->PVy)*(DInfo->vtxY[j]-EvtInfo->PVy))*TMath::Sqrt((DInfo->vtxX[j]-EvtInfo->PVx)*(DInfo->vtxX[j]-EvtInfo->PVx)+(DInfo->vtxY[j]-EvtInfo->PVy)*(DInfo->vtxY[j]-EvtInfo->PVy))/DInfo->dlerror2D[j] < 2.0 ) continue;
                //if (TMath::Sqrt((DInfo->vtxX[j]-EvtInfo->PVx)*(DInfo->vtxX[j]-EvtInfo->PVx)+(DInfo->vtxY[j]-EvtInfo->PVy)*(DInfo->vtxY[j]-EvtInfo->PVy)+(DInfo->vtxZ[j]-EvtInfo->PVz)*(DInfo->vtxZ[j]-EvtInfo->PVz))*TMath::Sqrt((DInfo->vtxX[j]-EvtInfo->PVx)*(DInfo->vtxX[j]-EvtInfo->PVx)+(DInfo->vtxY[j]-EvtInfo->PVy)*(DInfo->vtxY[j]-EvtInfo->PVy)+(DInfo->vtxZ[j]-EvtInfo->PVz)*(DInfo->vtxZ[j]-EvtInfo->PVz))/DInfo->dlerror[j] < 3.0) continue; 
   //             if (DInfo->alpha[j] > 1.) continue;
   //            if (DInfo->alpha2D[j] > 1.) continue;
               //if (DInfo->mass[j]>1.86484+0.15) continue;
                //if (DInfo->mass[j]<1.86484-0.15) continue;
                if(fabs(b4P->Rapidity())>2.0) continue;
              //if(DInfo->pt[j]>2.0) continue;
  ...
  void fillDTree(TVector3* bP, TVector3* bVtx, TLorentzVector* b4P, TVector3* boost, TVector3* D3Vec, int j, int typesize, bool REAL, EvtInfoBranches *EvtInfo, VtxInfoBranches *VtxInfo, TrackInfoBranches *TrackInfo, DInfoBranches *DInfo, GenInfoBranches *GenInfo)
  {
    ...
    dlos2D[typesize] = DInfo->rvtxMag[j]*DInfo->rvtxMag[j]/DInfo->sigmaRvtxMag[j]; //Dd0[typesize]*Dd0[typesize]/Dd0Err[typesize];
    dlos[typesize] = DInfo->lvtxMag[j]*DInfo->lvtxMag[j]/DInfo->sigmaLvtxMag[j]; //Ddxyz[typesize]*Ddxyz[typesize]/DdxyzErr[typesize];
    ...
    Dalpha2D[typesize] = DInfo->alpha2D[j];
    ...
    if(DInfo->type[j]==1||DInfo->type[j]==2||DInfo->type[j]==3||DInfo->type[j]==4||DInfo->type[j]==5||DInfo->type[j]==6||DInfo->type[j]==15||DInfo->type[j]==16)
      {
        ...
        dzos1[typesize] = Dtrk1Dz1[typesize]/Dtrk1DzError1[typesize];
        dxyos1[typesize] = Dtrk1Dxy1[typesize]/Dtrk1DxyError1[typesize];
        ...
        Dtrk1Chi2n[typesize] = TrackInfo->chi2n[DInfo->rftk1_index[j]];
        ...
        dzos2[typesize] = Dtrk2Dz1[typesize]/Dtrk2DzError1[typesize];
        dxyos2[typesize] = Dtrk2Dxy1[typesize]/Dtrk2DxyError1[typesize];
        ...
        Dtrk2Chi2n[typesize] = TrackInfo->chi2n[DInfo->rftk2_index[j]];
...
//cesar BDT
          float gbrVals_[50];
            ...
            gbrVals_[0] = Dpt[typesize];;
            gbrVals_[1] = Dy[typesize];
            gbrVals_[2] = Dchi2cl[typesize];
            gbrVals_[3] = dlos[typesize];
            gbrVals_[4] = dlos2D[typesize]; //dlos2D
            gbrVals_[5] = Ddxyz[typesize];
            gbrVals_[6] = Dalpha[typesize];
            gbrVals_[7] = Dalpha2D[typesize]; //alpha2D
            gbrVals_[8] = dzos1[typesize];
            gbrVals_[9] = dzos2[typesize];
            gbrVals_[10] = dxyos1[typesize];
            gbrVals_[11] = dxyos2[typesize];
            gbrVals_[12] = Dtrk1StripHit[typesize]+Dtrk1PixelHit[typesize];
            gbrVals_[13] = Dtrk2StripHit[typesize]+Dtrk2PixelHit[typesize];
            gbrVals_[14] = Dtrk1Pt[typesize];
            gbrVals_[15] = Dtrk2Pt[typesize];
            gbrVals_[16] = Dtrk1Eta[typesize];
            gbrVals_[17] = Dtrk2Eta[typesize];
            gbrVals_[18] = Dtrk1PtErr[typesize];
            gbrVals_[19] = Dtrk2PtErr[typesize];
          auto gbrVal = EvtInfo->forestBDT->GetClassifier(gbrVals_);
    Dmva[typesize] = gbrVal;
