...
{//{{{
    ...
    private:
        ...
        bool isCentrality_;
        int centrality;
        edm::EDGetTokenT<int> centSrc_;//Milan
        edm::EDGetTokenT<reco::Centrality> tok_centSrc_;
//        edm::EDGetTokenT<reco::Centrality> tok_centSrc_;//Milan        
        //edm::EDGetTokenT<edm::HepMCProduct> epAngleSrc_;
        edm::Handle<int> cbin_;
        edm::EDGetTokenT<reco::EvtPlaneCollection> tok_eventplaneSrc_;
/*
        std::string forestLabel_;
        GBRForest * forest_cent0to30_;//cesar  
        GBRForest * forest_cent30to50_;//cesar        
        GBRForest * forest_cent50to80_;//cesar 
        bool useForestFromDB_;
        std::vector<float> mvaVals_;
        std::string dbFileNameCent0to30_;//cesar
        std::string dbFileNameCent30to50_;//cesar
        std::string dbFileNameCent50to80_;//cesar 
        TH2D* hist_bdtcut_cent0to30;//cesar
        TH2D* hist_bdtcut_cent30to50;//cesar
        TH2D* hist_bdtcut_cent50to80;//cesar 
*/
...
{//{{{
    ...
    if(isCentrality_)
    {
      ...
      tok_centSrc_ = consumes<reco::Centrality>(iConfig.getParameter<edm::InputTag>("hfsumSrc"));
    } 
      //epAngleSrc_ = consumes<edm::HepMCProduct>(iConfig.getParameter<edm::InputTag>("epAngleSrc"));
      tok_eventplaneSrc_ = consumes<reco::EvtPlaneCollection>(iConfig.getParameter<edm::InputTag>("eventplaneSrc"));
/*
    forestLabel_ = iConfig.getParameter<std::string>("GBRForestLabel");
    dbFileNameCent0to30_ = iConfig.getParameter<std::string>("GBRForestFileNameCent0to30");
    dbFileNameCent30to50_ = iConfig.getParameter<std::string>("GBRForestFileNameCent30to50");
    dbFileNameCent50to80_ = iConfig.getParameter<std::string>("GBRForestFileNameCent50to80");
    useForestFromDB_ = false;
    TString bdtcut_filename_cent0to30;//cesar
    bdtcut_filename_cent0to30 = iConfig.getParameter<string>("BDTCutFileName_cent0to30");//cesar
    hist_bdtcut_cent0to30 = 0;//cesar
    TString bdtcut_filename_cent30to50;//cesar
    bdtcut_filename_cent30to50 = iConfig.getParameter<string>("BDTCutFileName_cent30to50");//cesar 
    hist_bdtcut_cent30to50 = 0;//cesar
    TString bdtcut_filename_cent50to80;//cesar
    bdtcut_filename_cent50to80 = iConfig.getParameter<string>("BDTCutFileName_cent50to80");//cesar
    hist_bdtcut_cent50to80 = 0;//cesar
      if(!bdtcut_filename_cent0to30.IsNull())//cesar
      {
        edm::FileInPath fip_cent0to30(Form("Bfinder/Bfinder/data/%s",bdtcut_filename_cent0to30.Data()));
        TFile ff_cent0to30(fip_cent0to30.fullPath().c_str(),"READ");
        hist_bdtcut_cent0to30 = (TH2D*)ff_cent0to30.Get("hist_bdtcut");
        ff_cent0to30.Close();
      }
      if(!bdtcut_filename_cent30to50.IsNull())//cesar
      {
        edm::FileInPath fip_cent30to50(Form("Bfinder/Bfinder/data/%s",bdtcut_filename_cent30to50.Data()));
        TFile ff_cent30to50(fip_cent30to50.fullPath().c_str(),"READ");
        hist_bdtcut_cent30to50 = (TH2D*)ff_cent30to50.Get("hist_bdtcut");
        ff_cent30to50.Close();
      }
      if(!bdtcut_filename_cent50to80.IsNull())//cesar
      {
        edm::FileInPath fip_cent50to80(Form("Bfinder/Bfinder/data/%s",bdtcut_filename_cent50to80.Data()));
        TFile ff_cent50to80(fip_cent50to80.fullPath().c_str(),"READ");
        hist_bdtcut_cent50to80 = (TH2D*)ff_cent50to80.Get("hist_bdtcut");
        ff_cent50to80.Close();
      }

     if(!useForestFromDB_){
      edm::FileInPath fipCent0to30(Form("Bfinder/Bfinder/data/%s",dbFileNameCent0to30_.c_str()));
      TFile gbrfileCent0to30(fipCent0to30.fullPath().c_str(),"READ");
      forest_cent0to30_ = (GBRForest*)gbrfileCent0to30.Get(forestLabel_.c_str());
      gbrfileCent0to30.Close();

      edm::FileInPath fipCent30to50(Form("Bfinder/Bfinder/data/%s",dbFileNameCent30to50_.c_str()));
      TFile gbrfileCent30to50(fipCent30to50.fullPath().c_str(),"READ");
      forest_cent30to50_ = (GBRForest*)gbrfileCent30to50.Get(forestLabel_.c_str());
      gbrfileCent30to50.Close();

      edm::FileInPath fipCent50to80(Form("Bfinder/Bfinder/data/%s",dbFileNameCent50to80_.c_str()));
      TFile gbrfileCent50to80(fipCent50to80.fullPath().c_str(),"READ");
      forest_cent50to80_ = (GBRForest*)gbrfileCent50to80.Get(forestLabel_.c_str());
      gbrfileCent50to80.Close();
    }//cesar
*/
...
//Centrality
     edm::Handle<int> occ;
     iEvent.getByToken(centSrc_,occ);
     int centrality;
     centrality = *occ;
    std::cout<<"hiBin = "<<centrality<<std::endl;

     if (centrality <= 60) return;
    ...
    //std::cout<<"zdc sum = "<<cent->zdcSum()<<std::endl;
...
//Milan EvtPlane
    ...
    for (EvtPlaneCollection::const_iterator rp = eventplanes->begin();rp !=eventplanes->end(); rp++) {
     std::cout<<rp->indx()<<" "<<rp->angle(2)<<" "<<rp->angle(2)<<std::endl;
    ...
    //EvtInfo.nTrgBook= N_TRIGGER_BOOKINGS;
      ...
      EvtInfo.HFsumET = HFsumET;
      EvtInfo.ephfpAngle[0] = ephfp1.angle(2);
      EvtInfo.ephfpAngle[1] = ephfp2.angle(2);
      EvtInfo.ephfpAngle[2] = ephfp3.angle(2);

/*
      cout <<"ephfpAngle = "<<ephfp2.angle(0)<< " "<<ephfp2.angle(1)<<" "<<ephfp2.angle(2)<<std::endl;
      cout <<"ephfmAngle = "<<ephfm2.angle(0)<< " "<<ephfm2.angle(1)<<" "<<ephfm2.angle(2)<<std::endl;
      cout <<"eptkAngle = "<<eptk2.angle(0)<< " "<<eptk2.angle(1)<<" "<<eptk2.angle(2)<<" "<<eptk2.angle(3)<<std::endl;
        
      cout <<"ephfpQ = "<<ephfp2.q(0)<< " "<<ephfp2.q(1)<<" "<<ephfp2.q(2)<<std::endl;
      cout <<"ephfmQ = "<<ephfm2.q(0)<< " "<<ephfm2.q(1)<<" "<<ephfm2.q(2)<<std::endl;
      cout <<"eptkQ = "<<eptk2.q(0)<< " "<<eptk2.q(1)<<" "<<eptk2.q(2)<<" "<<eptk2.q(3)<<std::endl;
*/
...
/*
//Cesar BDT
      if(centrality<60){EvtInfo.forestBDT = forest_cent0to30_;}//cesar
      else if(60<=centrality && centrality<100){EvtInfo.forestBDT = forest_cent30to50_;}//cesar
      else {EvtInfo.forestBDT = forest_cent50to80_;}//cesar
*/
            ...
            if (printInfo_) std::cout << "Got " << input_tracks.size() << " (" << input_losttracks.size() << " from lostTracks) tracks" << std::endl;
                if (input_tracks.size() > 0){
                    ...
                    for(std::vector<pat::PackedCandidate>::const_iterator tk_it=input_tracks.begin();
                        ...
                        i_tk++;
                        //std::cout<<i_tk-1<<" pf pt = "<<tk_it->pt()<<" chi2 = "<<(*chi2Map)[pfCandidates->ptrAt(i_tk-1)]<<std::endl;//" "<<trk.chi2()<<" chi2n = "<<trk.normalizedChi2()<<endl;
                        ...
                        if(doTkPreCut_){
                          ...
                          if( tk_it->pseudoTrack().ptError()/tk_it->pt()>0.1 ) continue;
                          //if( tk_it->pseudoTrack().hitPattern().numberOfValidHits()<11) continue;
                          //double chi2n=(*chi2Map)[pfCandidates->ptrAt(i_tk-1)];
                          //chi2n = chi2n/tk_it->pseudoTrack().hitPattern().trackerLayersWithMeasurement();
                          //if(  bool((*chi2Map)[pfCandidates->ptrAt(i_tk-1)]) && chi2n  >0.18) continue; 
                 ...
                 // float lambdaC_mass_window[2] = {0,10};
				  if(Dchannel_[14] == 1){
                        ...
                        */
                        ...
                        //PermuVec = DelDuplicate(PermuVec);
                        //for(unsigned int i = 0; i < PermuVec.size(); i++){
                            Dfinder::BranchOutNTk( DInfo, input_tracks, thePrimaryV,theBeamSpotV,lst,lstXYZP2, D_counter, lambdaC_mass_window, -1, -1, false, false, 15, 4);
                        //}
                    }
                   ///////////////////////////////////////////////////////////////////////////
                ...
                for(std::vector<pat::PackedCandidate>::const_iterator tk_it=input_tracks.begin();
                    ...
                    // if(readDedx_) {
                    //TrackInfo.dedx           [TrackInfo.size] = tk_it->pseudoTrack().dEdx();
                    // }else 
                        TrackInfo.dedx           [TrackInfo.size] = -1;
                          double chi2n=(*chi2Map)[pfCandidates->ptrAt(tk_hindex)];
                          chi2n = chi2n/tk_it->pseudoTrack().hitPattern().trackerLayersWithMeasurement();
                    TrackInfo.chi2n           [TrackInfo.size] = chi2n;
...
//Milan added
    ...
    const float mPiKCutMin = 1.72;
    const float mPiKCutMax = 2.01;
        ...
        for(int tk1idx = 0; tk1idx < (int)isNeededTrackIdx.size(); tk1idx++){
            ...
            for(int tk2idx = tk1idx+1; tk2idx < (int)isNeededTrackIdx.size(); tk2idx++){
                ...
                if(input_tracks[tk2_hindex].charge()*input_tracks[tk1_hindex].charge()>0) continue; // Milan added whole line 
                if(input_tracks[tk2_hindex].charge()*TkMassCharge[1].first<0) continue;
                if(tk2_hindex==tk1_hindex) continue;
//Milan added
      reco::TransientTrack posTT(input_tracks[tk1_hindex].pseudoTrack(), &(*bField) );
      reco::TransientTrack negTT(input_tracks[tk2_hindex].pseudoTrack(), &(*bField) );

      FreeTrajectoryState posState = posTT.impactPointTSCP().theState();
      FreeTrajectoryState negState = negTT.impactPointTSCP().theState();
      ...
      if (dca < 0. || dca > 999) continue;
      TrajectoryStateClosestToPoint posTSCP =
        posTT.trajectoryStateClosestToPoint( cxPt );
      TrajectoryStateClosestToPoint negTSCP =
        negTT.trajectoryStateClosestToPoint( cxPt );
      ...
      if( (mass1 > mPiKCutMax || mass1 < mPiKCutMin) && (mass2 > mPiKCutMax || mass2 < mPiKCutMin)) continue; //Milan June
      //if( totalPt < 1.0 ) continue; //Milan June
//end Milan added
...
///////here have to match the permutatiion with the track mass
        ...
        //if (unfitted_tktk_4vec.Mag()<2.3 || unfitted_tktk_4vec.Mag()>2.4) continue;
...
/////new version			
			...
			if (tkTT3.isValid())
			{
				...
				pushbackTrkIdx.push_back(selectedTkhidxSet[i].index_tk3);
                    ...
                    //std::cout<<"invariant mass1 = "<<invmass<<std::endl;
                    //if(invmass>2.0 && invmass<2.5) continue;
                    ...
                    //std::cout<<"invariant mass1 = "<<invmass<<" "<<tktk_4vec.Mag()<<" "<<unfitted_tktk_4vec.Mag()<<std::endl;
...
//Milan decay length significance
        ...
        SVector6 v11(thePrimaryV.covariance(0,0), thePrimaryV.covariance(0,1), thePrimaryV.covariance(1,1), 0., 0., 0.);
        SVector6 v21(tktk_VFPvtx->vertexState().error().cxx(), tktk_VFPvtx->vertexState().error().cyx(), tktk_VFPvtx->vertexState().error().cyy(), 0., 0., 0.);
        SMatrixSym3D sv11(v11);
        SMatrixSym3D sv21(v21);
        SMatrixSym3D totalCov2D = sv11 + sv21;
        SVector3 distanceVector2D(tktk_VFPvtx->position().x()-thePrimaryV.position().x(),tktk_VFPvtx->position().y()-thePrimaryV.position().y(),0);
        ...
        DInfo.dlerror2D[DInfo.size]       = sqrt(ROOT::Math::Similarity(totalCov2D, distanceVector2D));
        ...
        DInfo.rvtxMag[DInfo.size]         = d0LineOfFlight.perp();
        DInfo.sigmaRvtxMag[DInfo.size]    = sqrt(ROOT::Math::Similarity(d0TotalCov, distanceVector2D));

        DInfo.ip3d[DInfo.size]            = cur3DIP.value();
        DInfo.ip3derr[DInfo.size]         = cur3DIP.error();

//end Milan decay length significance
...
//Milan added alpha2D
        TVector3 svpvVec2D;
        svpvVec2D.SetXYZ(DInfo.vtxX[DInfo.size]-EvtInfo.PVx, DInfo.vtxY[DInfo.size]-EvtInfo.PVy, 0);
        TVector3 dVec2D;
        dVec2D.SetXYZ(DInfo.px[DInfo.size], DInfo.py[DInfo.size], 0);
        DInfo.alpha2D[DInfo.size] = svpvVec2D.Angle(dVec2D);
//Milan alpha2D end
