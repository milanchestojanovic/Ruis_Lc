...
//Milan
  ...
  */
  ...
  float   alpha_PV_2D[MAX_XB];//this first save the 3D alpha to test
  float   alpha_BS_2D[MAX_XB];
  float   alpha_BS_2D_correction[MAX_XB];
  float   Dip3d[MAX_XB];
  float   Dip3derr[MAX_XB];
  float   Dip2d_BS[MAX_XB];
  float   Dip2d_BS_err[MAX_XB];
 ...
 //DInfo.tktkResInfo
  ...
  void buildDBranch(TTree* dnt, bool D0kpimode=false, bool detailMode=false)
  {
    ...
    */
	...
	dnt->Branch("alpha_PV_2D",alpha_PV_2D,"alpha_PV_2D[Dsize]/F");
	dnt->Branch("alpha_BS_2D",alpha_BS_2D,"alpha_BS_2D[Dsize]/F");
	dnt->Branch("alpha_BS_2D_correction",alpha_BS_2D_correction,"alpha_BS_2D_correction[Dsize]/F");
	dnt->Branch("ImpactParameter3D",Dip3d,"Dip3d[Dsize]/F");
	dnt->Branch("ImpactParameter3Derror",Dip3derr,"Dip3derr[Dsize]/F");
	//dnt->Branch("ImpactParameter2D_BS",Dip2d_BS,"Dip2d_BS[Dsize]/F");
	//dnt->Branch("ImpactParameter2Derror_BS",Dip2d_BS_err,"Dip2d_BS_err[Dsize]/F");
    ...
    if(!D0kpimode)
      {
        ...
        dnt->Branch("Dtk1mass_Dfindersave",Dtk1mass_Dfindersave,"Dtk1mass_Dfindersave[Dsize]/F");
		dnt->Branch("Dtk2mass_Dfindersave",Dtk2mass_Dfindersave,"Dtk2mass_Dfindersave[Dsize]/F");
		dnt->Branch("Dtk3mass_Dfindersave",Dtk3mass_Dfindersave,"Dtk3mass_Dfindersave[Dsize]/F");
		dnt->Branch("Dtk4mass_Dfindersave",Dtk4mass_Dfindersave,"Dtk4mass_Dfindersave[Dsize]/F");
...
///////
  ...
  {//{{{
    ...
    for(int t=0; t<8;t++) Dtypesize[t]=0;
    for(int t=0;t<16;t++)
      {
        if(t%2==0)
          {
    ...
    for(int j=0;j<GenInfo->size;j++)
      {
		...
		//cout<<"good1"<<endl;
		//cout<<"get data"<<GenInfo->da2[GenInfo->da1[j]]<<endl;
		//int tmp1=GenInfo->da2[GenInfo->da1[j]];
		//cout<<"get pdgId"<<GenInfo->pdgId[tmp1]<<endl;
		//cout<<"gsize"<<endl;
		//cout<<"gsize check"<<Gdesecent12pdgId[gsize]<<endl;
		//int tmp=GenInfo->pdgId[GenInfo->da2[GenInfo->da1[j]]];
		//cout<<"check"<<GenInfo->pdgId[GenInfo->da2[GenInfo->da1[j]]]<<endl;
		//int tmp=GenInfo->pdgId[GenInfo->da2[GenInfo->da1[j]]];
		//cout<<"first "<< GenInfo->da1[j]<<endl;
		//cout<<"second "<< GenInfo->da2[GenInfo->da1[j]]<<endl;
		//cout<<"assign "<<endl;
		//Gdesecent12pdgId[gsize]=tmp;
...
///here I want to add the part for channel 15 and 16. 
          ...
          {
			 // cout<<"nDa first: "<<GenInfo->nDa[DgenIndex[j]]<<endl;
			 // cout<<"DgenIndex: "<<DgenIndex[j]<<endl;
  ...
  void fillDTree(TVector3* bP, TVector3* bVtx, TLorentzVector* b4P, int j, int typesize, bool REAL, EvtInfoBranches *EvtInfo, VtxInfoBranches *VtxInfo, TrackInfoBranches *TrackInfo, DInfoBranches *DInfo, GenInfoBranches *GenInfo)
  {
	...
	//Dip2d_BS[typesize] = DInfo->ip2d_BS[j];
	//Dip2d_BS_err[typesize] = DInfo->ip2d_BS_err[j];
	...
	////here will add the new branch to test and add the branch with respect with BS
	TVector3 svpvVec;
	//svpvVec.SetXYZ(DInfo->vtxX[j]-EvtInfo->PVx, DInfo->vtxY[j]-EvtInfo->PVy, DInfo->vtxZ[j]-EvtInfo->PVz);
	svpvVec.SetXYZ(DInfo->vtxX[j]-EvtInfo->PVx, DInfo->vtxY[j]-EvtInfo->PVy, 0);
	TVector3 dVec;
	//dVec.SetXYZ(DInfo->px[j], DInfo->py[j], DInfo->pz[j]);
	dVec.SetXYZ(DInfo->px[j], DInfo->py[j], 0);
	alpha_PV_2D[typesize] = svpvVec.Angle(dVec);
	TVector3 svbsVec;
	svbsVec.SetXYZ(DInfo->vtxX[j]-EvtInfo->BSx, DInfo->vtxY[j]-EvtInfo->BSy, 0);
	TVector3 dVec_for2D;
	dVec_for2D.SetXYZ(DInfo->px[j], DInfo->py[j], 0);
	alpha_BS_2D[typesize] = svbsVec.Angle(dVec_for2D);
	////the following for the correct but not sure correct or not.
	...
	alpha_BS_2D_correction[typesize] = svbsVec_corr.Angle(dVec_for2D);
    ...
    */
    ...
    //DInfo.trkInfo
    float trk1mass,trk2mass,trk3mass;
    if(DInfo->type[j]==1||DInfo->type[j]==2||DInfo->type[j]==3||DInfo->type[j]==4||DInfo->type[j]==5||DInfo->type[j]==6||DInfo->type[j]==15||DInfo->type[j]==16)
      {
		...
		Dtk1mass_Dfindersave[typesize] = DInfo->rftk1_mass[j];
		...
		Dtk2mass_Dfindersave[typesize] = DInfo->rftk2_mass[j];
        ...
        if(DInfo->type[j]==1||DInfo->type[j]==2)
          {
			...
			Dtk3mass_Dfindersave[typesize] = 0;
            ...
            //Dtrk4dedx[typesize] = -20;
          	Dtk4mass_Dfindersave[typesize] = 0;
        ...
        else if(DInfo->type[j]==3||DInfo->type[j]==4||DInfo->type[j]==15||DInfo->type[j]==16)
          {
            ...
            Dtk3mass_Dfindersave[typesize] = DInfo->rftk3_mass[j];
    ...
    else if(DInfo->type[j]==7||DInfo->type[j]==8||DInfo->type[j]==9||DInfo->type[j]==10||DInfo->type[j]==11||DInfo->type[j]==12||DInfo->type[j]==13||DInfo->type[j]==14)
      {
        ...
        Dtrk1Y[typesize] = b4P->Rapidity();
        Dtrk1P[typesize] = b4P->P();
        Dtrk1Dxy[typesize] = TrackInfo->dxy[DInfo->rftk2_index[j]];
   ...
   */ 
    ...
    //if((DInfo->type[j]==7||DInfo->type[j]==8)&&!lambdaCpkpi) RpdgId=PHI_PDGID;
    //else if((DInfo->type[j]==9||DInfo->type[j]==10||DInfo->type[j]==11||DInfo->type[j]==12||DInfo->type[j]==13||DInfo->type[j]==14)&& !lambdaCpkpi) RpdgId=DZERO_PDGID;
	/*
				                           ...
				                           { 
					                             if(TMath::Abs(GenInfo->pdgId[TrackInfo->geninfo_index[DInfo->rftk1_index[j]]])==DInfo->rftk1_MassHypo[j] &&
					                                ...
					                                TMath::Abs(GenInfo->pdgId[TrackInfo->geninfo_index[DInfo->rftk3_index[j]]])==DInfo->rftk3_MassHypo[j])
					                               {
													   ...
													   {
                                               std::cout<<TMath::Abs(GenInfo->pdgId[TrackInfo->geninfo_index[DInfo->rftk1_index[j]]])<<" "<<" "<<TrackInfo->geninfo_index[DInfo->rftk1_index[j]] <<" "<<DInfo->rftk1_index[j] <<" " <<DInfo->rftk1_MassHypo[j]<<std::endl;
                                               std::cout<<TMath::Abs(GenInfo->pdgId[TrackInfo->geninfo_index[DInfo->rftk2_index[j]]])<<" "<<" "<<TrackInfo->geninfo_index[DInfo->rftk2_index[j]] <<" "<<DInfo->rftk2_index[j] <<" " <<DInfo->rftk2_MassHypo[j]<<std::endl;
                                               std::cout<<TMath::Abs(GenInfo->pdgId[TrackInfo->geninfo_index[DInfo->rftk3_index[j]]])<<" "<<" "<<TrackInfo->geninfo_index[DInfo->rftk3_index[j]] <<" "<<DInfo->rftk3_index[j] <<" " <<DInfo->rftk3_MassHypo[j]<<std::endl;
                                               std::cout<<__LINE__<<std::endl;
							                                 Dgen[typesize] = 23344;
							                                 dGenIdxRes = GenInfo->mo1[TrackInfo->geninfo_index[DInfo->rftk1_index[j]]];
															 Dgensubchannel[typesize] = 1; //for pkpi channel.
